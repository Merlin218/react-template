...
const request = (AxiosConfig: AxiosRequestConfig, customConfig: CustomConfigType) => {
	// 设置请求头和请求路径
		...
		(res: AxiosResponse<responseType, any>) => {
			...
			if (res.data.code === 401) {
				...
				customConfigs.loading &&
					message.success({
						content: (customConfigs.useMsgFromEnd && res.data.message) ?? customConfigs.successTitle,
						duration: 2,
					});
...
const createRequest = (config: AxiosRequestConfig, customConfig?: CustomConfigType) => {
